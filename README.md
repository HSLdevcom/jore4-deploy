...
repositories) for changes. Whenever there's a change, it automatically (re)deploys the
desired resources to the cluster. Note that Flux also stores its own components and configurations
as Kubernetes resources, so it also automatically updates its on configuration the same way as it
can redeploy applications.

Flux automatically accesses the cluster from the instead without having to allow an IP address
...
there's a change, it automatically reconciles. As a backdraw, this also means that we cannot
(easily) manually deploy a desired version to the cluster, we always have to configure the desired
version in the git/helm/docker repositories.
